...
the venue documents propagation and finality guarantees. Every conclusion is
scoped to the listed product, transport, API surface, and 2026-08-06 documentation
snapshot; it is not a venue-wide contract.

| Venue | Product | Protocol / flow | API version / snapshot | Evidence type | Exact unknown/ambiguous signal | Fields in the error/ack | Documented or defensible client recipe | Same-request idempotency probe |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Binance | Spot | REST placement/query plus User Data Stream | REST `/api/v3`; public docs snapshot 2026-08-06 | Explicit unknown code and documented recovery direction | `-1007 TIMEOUT`: “Send status unknown; execution status unknown.” HTTP `5XX` responses can also have an unknown execution state. | The timeout format documents only `code` and `msg`; it does not echo the submitted `newClientOrderId`, signing `timestamp`, or an order ID. | For this Spot REST surface, retain `symbol + newClientOrderId`; consume the User Data Stream, then query order status with `origClientOrderId`. This is the only explicit unknown-outcome recipe in the set. | No for this surface. `newClientOrderId` is unique only among open orders and may be reused after fill, so repeating placement can create a second order. |
| Kraken | Spot | REST `AddOrder` and order queries; WebSocket v2 ack is supplementary evidence | REST `/0` plus WebSocket v2; public docs snapshot 2026-08-06 | Generic service errors; recovery inferred from documented lookup surfaces | No REST placement-specific unknown-outcome code. `EService:Unavailable`, `EService:Busy`, `EGeneral:Internal error`, and non-JSON gateway errors are temporary/degraded-service signals without placement-finality semantics. | REST `AddOrder` success returns a venue `txid`; the reviewed REST error contract does not echo `nonce`, `cl_ord_id`, a request ID, or a server timestamp. | For the reviewed Spot surfaces, persist a unique `cl_ord_id`; reconcile both open and closed orders using its filters before considering another placement. This is an inference from the available lookup surfaces, not Kraken's generic retry advice. | No documented probe for these surfaces. REST nonces must increase, and `cl_ord_id` uniqueness is guaranteed only across open orders. A completed first order can therefore escape duplicate protection. |
| Hyperliquid | Perpetual order action (shared exchange surface also covers spot) | HTTPS exchange action plus Info query and WebSocket `orderUpdates` | Public Exchange/Info APIs; no semantic version stated; snapshot 2026-08-06 | No dedicated code; query/stream contract supports an inferred recipe | No documented outcome-unknown code on the reviewed exchange action. Semantic placement failures are per-order string errors in an otherwise successful exchange response. | A successful or rejected item can carry order status/error information; no dedicated ambiguous error with echoed request identifiers is documented. | For the reviewed exchange surface, assign a 128-bit `cloid`; query `orderStatus` by that ID and consume `orderUpdates`. `unknownOid` has no documented propagation/finality window, so it is not by itself conclusive. | No documented probe for this surface. Transaction nonces are anti-replay while retained and must not be reused; API-wallet nonce state can be pruned after deregistration, expiry, or loss of funds on the registering account. Same-`cloid` replay is not documented as idempotent. |
| Lighter | Perpetuals | REST `sendTx` to sequencer, REST status queries, and WebSocket order updates | REST `/api/v1`; OpenAPI semantic version unstated; snapshot 2026-08-06 | Explicit two-stage acceptance plus generic errors and query/stream contracts | Two-stage acceptance is explicit: `code=200` does not guarantee execution because the sequencer may still reject. Generic `29501 process timeout` and `29500 internal server error` have no documented enqueue/finality semantics. | A successful `sendTx` returns `tx_hash`; orders and streams expose `client_order_index`, venue order index, nonce, and status. The generic errors do not document these as echoed fields. | As a conservative client convention, assign a `client_order_index` unique within the account's full reconciliation window; monitor WebSocket order updates and query `/accountOrders` by that index. If a `tx_hash` was received, query `/tx` for its status. `/accountOrders` supports the last 10,000 active orders without a time limit, or the last 1,000 inactive orders from 24 hours; negative lookups have no documented finality. | No documented probe for these surfaces. `21728 client order index already exists` detects some duplicates, but the docs do not promise replay returns the original result; `21104 invalid nonce` rejects nonce reuse. |
| Extended | Perpetuals | REST `/api/v1` asynchronous placement/query plus account WebSocket updates | REST v1; public docs snapshot 2026-08-06 | Explicit asynchronous acceptance plus generic errors and query/stream contracts | No dedicated outcome-unknown code on the reviewed surface. REST placement is asynchronous: an accepted request can later be rejected or cancelled by the matching engine. Generic `500 InternalServerError` / `1006 UnhandledError` do not define placement finality. | Placement success returns Extended's order ID and the caller's `externalId`; the generic errors do not document either ID or the request nonce as echoed. | For the reviewed perpetual surface, persist a caller-assigned order `id` that is attempt-unique for the full reconciliation window; it is returned as `externalId`. Consume account WebSocket updates and query `/api/v1/user/orders/external/{externalId}`. Treat zero or multiple rows as unresolved. Resolve only one row whose `externalId`, account, market, and immutable order context match the original attempt, then retain its Extended order ID, status, and `statusReason`. | No documented probe for this surface. `1134 DuplicateOrder` is documented, but repeating the create request is not promised to return the original result or be a safe status probe. |
...
idempotent replay receipt. The direct external-ID lookup returns an array, and
the docs do not define identifier uniqueness or reuse. Clients must therefore
keep the value attempt-unique for their full reconciliation window, treat zero
or multiple rows as unresolved, and validate a sole row against the original
account, market, and immutable order context. Extended also documents limited
archival for final zero-fill orders (seven days for regular accounts, none for
high-volume accounts), without stating whether the direct external-ID lookup has
a different retention guarantee.
...
8. Add a retained terminal create-result lookup under an attempt-unique
   ...
   Before dispatching to the matching engine, durably register that attempt key,
   the complete signed request, and an unresolved state. The registration may be
   acknowledged only after a storage watermark that survives the failure domain
   promised by the API; alternatively, bind the attempt mapping and matching-
   engine command atomically in one durable record with recovery for both. If
   registration or atomic binding fails, do not dispatch the command; return a
   pre-dispatch failure. Create recovery is deterministic only after this
   durability watermark. The lookup must return every field needed to
   reconstruct the create response,
